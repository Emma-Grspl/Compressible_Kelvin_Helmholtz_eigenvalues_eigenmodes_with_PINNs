...
class Mstab17SupersonicResult:
    """Supersonic shooting result with explicit legacy stage-2 semantics.

    ``mode_success`` and ``success`` are retained for API compatibility and
    remain aliases of the historical bounded-amplitude criterion.  Physical
    modal reconstruction is assessed later from reconstructed fields.
    """
    ...
    use_mapping: bool
    mapping_scale: float
    ln_p_start_right_exact: float = float("nan")
    legacy_stage2_bound_hit: bool = False
    legacy_mode_success: bool | None = None
    legacy_success: bool | None = None

    def __post_init__(self) -> None:
        # Keep direct construction with the historical field list compatible.
        if self.legacy_mode_success is None:
            self.legacy_mode_success = bool(self.mode_success)
        if self.legacy_success is None:
            self.legacy_success = bool(self.success)
...
class Mstab17SupersonicSolver:
    ...
    def solve_eigenvalue(
        ...
        *,
        ...
        initial_cr_min = float(max(cr_min, 0.0))
        initial_cr_max = float(cr_max)
        initial_ci_min = float(max(ci_min, self.ci_floor))
        initial_ci_max = float(ci_max)

        current_cr_min = initial_cr_min
        current_cr_max = initial_cr_max
        current_ci_min = initial_ci_min
        current_ci_max = initial_ci_max
        ...
        for _ in range(max_iter):
            ...
            if constrain_to_initial_box:
                current_cr_min = max(current_cr_min, initial_cr_min)
                current_cr_max = min(current_cr_max, initial_cr_max)
                current_ci_min = max(current_ci_min, initial_ci_min)
                current_ci_max = min(current_ci_max, initial_ci_max)

                if current_cr_min > current_cr_max:
                    current_cr_min = current_cr_max = best_cr
                if current_ci_min > current_ci_max:
                    current_ci_min = current_ci_max = best_ci
    ...
    def exact_right_log_amplitude(self, cr: float, ci: float) -> float:
        """Return the unbounded right log-amplitude matching ``ln|p|``.

        The right log-amplitude enters the integrated state additively, so no
        scalar minimization is needed.  This quantity may legitimately lie
        outside the historical ``[ln_p_right_min, ln_p_right_max]`` bounds.
        """

        sol_left, _, sol_right_full, _ = self.get_trajectories(
            cr,
            ci,
            ln_p_start_right=0.0,
        )
        if not (sol_left.success and sol_right_full.success):
            raise RuntimeError(
                "Trajectory failure while determining the exact right-hand "
                "pressure amplitude."
            )
        target_y = float(self.amplitude_match_y)
        ln_left = self._interp_component(target_y, sol_left, 2)
        ln_right_zero = self._interp_component(target_y, sol_right_full, 2)
        return float(ln_left - ln_right_zero)
    ...
    def solve(
        ...
        *,
        ...
        ln_p_start_right_exact = self.exact_right_log_amplitude(cr_star, ci_star)
        legacy_stage2_bound_hit = bool(
            ln_p_start_right_exact < self.ln_p_right_min
            or ln_p_start_right_exact > self.ln_p_right_max
        )
        ...
        legacy_mode_success = bool(stage2_err < 1e-2)
        legacy_success = bool(spectral_success and legacy_mode_success)
        return Mstab17SupersonicResult(
            ...
            mode_success=legacy_mode_success,
            success=legacy_success,
            use_mapping=self.use_mapping,
            mapping_scale=self.mapping_scale,
            ln_p_start_right_exact=ln_p_start_right_exact,
            legacy_stage2_bound_hit=legacy_stage2_bound_hit,
            legacy_mode_success=legacy_mode_success,
