...
python "${ROOT_DIR}/code/src/classical_solver/gep/merge_supersonic_mode_database_chunks.py" \
...
python "${ROOT_DIR}/code/scripts/legacy/analyze_supersonic_mode_database.py" \
  --surface-csv "${SURFACE_CSV}" \
  --output-dir "${PLOT_DIR}"

echo "Plots presentation a partir de ${SURFACE_CSV}"
python "${ROOT_DIR}/plots/scripts/pinn_supersonic/plot_supersonic_database_presentation.py" \
  --surface-csv "${SURFACE_CSV}" \
  --output-dir "${PLOT_DIR}"
