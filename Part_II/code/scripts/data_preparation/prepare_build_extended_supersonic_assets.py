...
EXTENSION_DEFAULT = Path(
    "classic_supersonic/reproducibility/results/dense_kappa_q_lowM_lowalpha_extension_v1"
)
OUTPUT_DEFAULT = Path(
    "assets/classic_supersonic/dense_kappa_q_campaign_v1_EXTENDED_ASSETS"
)
...
def load_extension_reference(extension_dir: Path) -> pd.DataFrame:
    ...
    frame = pd.read_csv(path)
    frame["reference_subset"] = "lowM_lowalpha_extension"
    frame["reference_source_path"] = str(path)
    return frame
...
def save_ci_map(
    ...
    # Classical neutral estimates, if present.
    if "neutral_alpha_estimate" in reference.columns:
        neutral_classic = reference[["Mach", "neutral_alpha_estimate"]].copy()
        neutral_classic["neutral_alpha_estimate"] = pd.to_numeric(
            neutral_classic["neutral_alpha_estimate"], errors="coerce"
        )
        neutral_classic = (
            neutral_classic.dropna()
            .groupby("Mach", as_index=False)["neutral_alpha_estimate"]
            .median()
            .sort_values("Mach")
        )
        if not neutral_classic.empty:
            ax.plot(
                ...
                marker=".",
                linewidth=1.2,
                label=r"Classical neutral estimate $c_i=0$",
                zorder=4,
            )
    ...
    ax.set_title(
        r"Extended classical supersonic reference and Blumen digitization"
    )
    ax.set_xlim(
        min(0.99, float(reference["Mach"].min()) - 0.01),
        max(2.02, float(reference["Mach"].max()) + 0.03),
    )
    ax.set_ylim(
        min(0.0, float(reference["alpha"].min()) - 0.005),
        max(0.36, float(reference["alpha"].max()) + 0.01),
    )
...
def build_modes_and_long_csv(
    ...
    with gzip.open(csv_tmp, "wt", newline="", encoding="utf-8") as csv_handle, \
        ...
        cover.text(
            0.5, 0.84,
            "Extended classical supersonic modal atlas",
        ...
        for path in roots:
            with np.load(path, allow_pickle=False) as payload:
                ...
                for raw_index in np.argsort(payload["alpha"]):
                    ...
                    summaries.append({
                        "extended_reference_id": ref_row["extended_reference_id"],
                        "Mach": Mach,
                        "alpha": alpha,
                        "cr": cr,
                        "ci": ci,
...
def main() -> int:
    ...
    point_csv = output_dir / "classical_supersonic_extended_reference.csv"
    ci_pdf = output_dir / "classical_supersonic_extended_ci_map.pdf"
    modes_pdf = (
        output_dir
        / "classical_supersonic_extended_all_modes_p_rho_u_v.pdf"
    )
    modes_long = (
        output_dir
        / "classical_supersonic_extended_modes_long.csv.gz"
    )
    modes_index = output_dir / "classical_supersonic_extended_modes_index.csv"
    ...
    (output_dir / "README.md").write_text(
        f"""# Extended classical supersonic assets
...
""",
    ...
    print("=== EXTENDED SUPERSONIC ASSETS ===")
