...
DEFAULT_VALIDATED_MACH_VALUES = (1.2, 1.3, 1.4, 1.5, 1.6, 1.7)
...
def load_fields_df_for_source_groups(source_groups: list[str] | tuple[str, ...]) -> pd.DataFrame:
    frames: list[pd.DataFrame] = []
    requested = {str(value) for value in source_groups}

    if "reference_core_local_modal" in requested:
        ref = pd.read_csv(REFERENCE_FIELDS_CSV).copy()
        ref["source_group"] = "reference_core_local_modal"
        frames.append(ref)
    if "M120_pointwise_core" in requested and M120_FIELDS_CSV.exists():
        m120 = pd.read_csv(M120_FIELDS_CSV).copy()
        m120["source_group"] = "M120_pointwise_core"
        frames.append(m120)
    if "M140_branch_guided_reconfirm" in requested and M140_FIELDS_CSV.exists():
        m140 = pd.read_csv(M140_FIELDS_CSV).copy()
        m140["source_group"] = "M140_branch_guided_reconfirm"
        frames.append(m140)
    if "M170_pointwise_corrected" in requested and M170_FIELDS_CSV.exists():
        m170 = pd.read_csv(M170_FIELDS_CSV).copy()
        m170["source_group"] = "M170_pointwise_corrected"
        frames.append(m170)

    if not frames:
        raise RuntimeError(f"No supersonic validated modal fields source is available for groups: {sorted(requested)}")
    return pd.concat(frames, ignore_index=True)
...
def load_strict_validated_points(mach_values: list[float] | tuple[float, ...] | None = None) -> pd.DataFrame:
    if mach_values is None:
        mach_values = DEFAULT_VALIDATED_MACH_VALUES
    points_df = load_validated_points([float(value) for value in mach_values])
    points_df = attach_corrected_blumen_errors(points_df)
    points_df = keep_strict_gold_rows(points_df)
    return points_df.sort_values(["Mach", "alpha"]).reset_index(drop=True)


def select_validated_modal_point(
    mach: float,
    alpha: float,
    *,
    ...
    mach_values: list[float] | tuple[float, ...] | None = None,
) -> pd.Series:
    if points_df is None:
        points_df = load_strict_validated_points(mach_values=mach_values)
    if points_df.empty:
        raise RuntimeError("No strict validated supersonic modal points are available.")

    work = points_df.copy()
    work["mach_abs_err"] = (work["Mach"].astype(float) - float(mach)).abs()
    work["alpha_abs_err"] = (work["alpha"].astype(float) - float(alpha)).abs()

    if tolerance_mach is not None:
        work = work.loc[work["mach_abs_err"] <= float(tolerance_mach)].copy()
    if tolerance_alpha is not None:
        work = work.loc[work["alpha_abs_err"] <= float(tolerance_alpha)].copy()
    if work.empty:
        raise RuntimeError(
            "No validated supersonic modal point satisfies the requested tolerances "
            f"for Mach={float(mach):.6f}, alpha={float(alpha):.6f}."
        )

    work = work.sort_values(["mach_abs_err", "alpha_abs_err", "Mach", "alpha"], kind="stable").reset_index(drop=True)
    return work.iloc[0]
...
def load_supersonic_validated_mode_reference(
    ...
    *,
    ...
    if points_df is None:
        if mach_values is None:
            mach_values = [float(mach)]
        points_df = load_strict_validated_points(mach_values=mach_values)
    row = select_validated_modal_point(
        mach=float(mach),
        alpha=float(alpha),
        points_df=points_df,
    ...
    if fields_df is None:
        fields_df = load_fields_df_for_source_groups([str(row["source_group"])])
    sub = point_fields(fields_df, float(row["alpha"]), float(row["Mach"]), str(row["source_group"]))
    if sub.empty:
        raise RuntimeError(
            "Validated supersonic modal fields are missing for "
            f"Mach={float(row['Mach']):.6f}, alpha={float(row['alpha']):.6f}, "
    ...
    arrays: dict[str, np.ndarray] = {
        ...
        "p_imag": p_imag,
        "p_abs": p_abs,
        "p_real_norm": p_real / norm,
        "p_abs_norm": p_abs / norm,
        "p_imag_norm": p_imag / norm,
    }
    optional_columns = ("q_real", "q_imag", "gamma_real", "gamma_imag")
    for column in optional_columns:
        if column in sub.columns:
            arrays[column] = sub[column].to_numpy(dtype=float)

    metadata = {
        "requested_mach": float(mach),
        "requested_alpha": float(alpha),
        "selected_mach": float(row["Mach"]),
        "selected_alpha": float(row["alpha"]),
        "mach_abs_err": float(abs(float(row["Mach"]) - float(mach))),
        "alpha_abs_err": float(abs(float(row["alpha"]) - float(alpha))),
        ...
        "stage1_mismatch": float(row["stage1_mismatch"]),
        "stage2_mismatch": float(row["stage2_mismatch"]),
        "status": str(row["status"]),
        "blumen_ci_corrected": float(row["blumen_ci_corrected"]),
        "err_ci_abs_corrected": float(row["err_ci_abs_corrected"]),
        "err_ci_rel_corrected": float(row["err_ci_rel_corrected"]),
        "normalization_max_abs_p": norm,
...
def main() -> None:
    ...
    points_df = load_strict_validated_points([float(value) for value in args.mach_values])
