...
def curve_intersections_at_mach(points: pd.DataFrame, mach: float) -> list[float]:
    sub = points[["Mach", "alpha", "_row_order"]].sort_values("_row_order").reset_index(drop=True)
    if len(sub) < 2:
        return []
...
def estimate_level_from_isolines(
    points: pd.DataFrame,
    mach: float,
    alpha: float,
    *,
    principal_branch: bool = False,
) -> float:
    ...
    for level, sub in points.groupby("level_value"):
        if principal_branch:
            sub = sub[["Mach", "alpha", "_row_order"]].sort_values("_row_order").reset_index(drop=True)
            if len(sub) < 2:
                continue
            mach_values = sub["Mach"].to_numpy(dtype=float)
            if mach < float(np.min(mach_values)) or mach > float(np.max(mach_values)):
                continue
            intersections = curve_intersections_at_mach(sub, mach)
            if not intersections:
                continue
            alpha_on_curve = float(max(intersections))
        else:
        ...
        rows.append(
            {
                ...
                "blumen_ci": estimate_level_from_isolines(
                    ci_points,
                    float(mach),
                    float(alpha),
