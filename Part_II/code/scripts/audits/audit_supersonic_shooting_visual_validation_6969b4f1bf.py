...
from scripts.audits.audit_supersonic_families_against_blumen import DEFAULT_OUTPUT_DIR  # noqa: E402
...
def reconstruct_shooting_fields(
    *,
    ...
    # Match the full complex pressure at the same location used
    # for the Riccati and amplitude matching.
    p_left_match = (
        np.exp(
    ...
    if abs(p_right_match) == 0.0:
        raise RuntimeError(
            "Zero right pressure at the shooting matching point."
        )

    complex_match_scale = (
        p_left_match / p_right_match
    )
    ...
    # Assemble at match_y, not at the historical hard-coded y=0.
    left_mask = (
        y_left < solver.match_y
    )
    right_mask = (
        y_right >= solver.match_y
    )
    ...
    # Exact first-order pressure variable from the Riccati field.
    p_y = gamma * p
    ...
    # normalize_fields applies one common complex normalization to
    # the primitive fields. Recover that same factor for q=p_y.
    p_before_normalization = p.copy()

    u, v, p, rho = normalize_fields(
        y=y,
        u=u,
        v=v,
        p=p,
