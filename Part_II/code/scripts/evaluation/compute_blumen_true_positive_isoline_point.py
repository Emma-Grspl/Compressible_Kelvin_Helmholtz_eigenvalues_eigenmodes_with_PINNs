...
if str(FROZEN_SOLVER_ROOT) not in sys.path:
    sys.path.insert(0, str(FROZEN_SOLVER_ROOT))
...
def load_seed_pool(reference_path: Path, pointwise: pd.DataFrame) -> pd.DataFrame:
    ...
    reference = reference[["Mach", "alpha", "cr", "ci"]].copy()
    ...
    pointwise_ok = pointwise_ok[
        ["Mach", "alpha", "classical_cr", "classical_ci"]
    ].copy()
    ...
    out.update(
        {
            ...
            "message": "Classical alpha root validated with the production solver.",
        }
    )
    return out
...
def main() -> int:
    ...
    parser.add_argument("--preferred-cr", type=float, default=None)
    parser.add_argument("--preferred-ci", type=float, default=None)
    ...
    try:
        preferred = None
        if args.preferred_cr is not None and args.preferred_ci is not None:
            preferred = {"cr": args.preferred_cr, "ci": args.preferred_ci}

        center = evaluator.evaluate(alpha_blumen, preferred=preferred)
        ...
        if exact is not None:
            ...
            # Near alpha=0, a fixed symmetric scan would skip the entire
            # interval between alpha_min and the first positive offset.
            if alpha_blumen < 2.0 * args.scan_step:
                micro_upper = min(
                    args.alpha_max,
                    max(2.0 * args.scan_step, 1.5 * alpha_blumen),
                )
                micro_grid = np.geomspace(
                    args.alpha_min,
                    micro_upper,
                    num=12,
                )
                for trial_alpha in micro_grid:
                    evaluator.evaluate(
                        float(trial_alpha),
                        preferred=center if center is not None else preferred,
                    )
                rows = evaluator.successful()
                exact = nearest_exact(rows, alpha_blumen, args.ci_tolerance)
            ...
            for step_index in range(1, max_steps + 1):
                if exact is not None or bracket is not None:
                    break
                radius = step_index * args.scan_step
                for trial_alpha in (
                    max(args.alpha_min, alpha_blumen - radius),
                    min(args.alpha_max, alpha_blumen + radius),
                ):
                    evaluator.evaluate(
                        trial_alpha,
                        preferred=center if center is not None else preferred,
    ...
    successful = evaluator.successful()
    sampled_alpha = [float(row["alpha"]) for row in successful]
    result["n_solver_calls"] = int(evaluator.n_solver_calls)
    result["delta_ci"] = result.get("delta_ci_at_root", np.nan)
    result["abs_delta_alpha"] = abs(float(result["delta_alpha"])) if np.isfinite(
        pd.to_numeric(pd.Series([result.get("delta_alpha")]), errors="coerce").iloc[0]
    ) else np.nan
    result["bracket_alpha_left"] = result.get("alpha_bracket_lower", np.nan)
    result["bracket_alpha_right"] = result.get("alpha_bracket_upper", np.nan)
    if not np.isfinite(
        pd.to_numeric(
            pd.Series([result.get("bracket_alpha_left")]), errors="coerce"
        ).iloc[0]
    ):
        result["bracket_alpha_left"] = min(sampled_alpha) if sampled_alpha else np.nan
    if not np.isfinite(
        pd.to_numeric(
            pd.Series([result.get("bracket_alpha_right")]), errors="coerce"
        ).iloc[0]
    ):
        result["bracket_alpha_right"] = max(sampled_alpha) if sampled_alpha else np.nan
