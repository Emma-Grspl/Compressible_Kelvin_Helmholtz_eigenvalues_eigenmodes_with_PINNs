...
VALIDATION_FILE = (
    REPO / 'assets/pinn_supersonic/csv/computational_cost/cost500/table_cost500_runtime_frame_500.csv'
)

CHECKPOINT_SOURCE_PREFIX = "assets/pinn_supersonic/atlas2d_v1_continuousM"

OUTPUT_ROOT = REPO / "experiments/pinn_direct/COST500"
...
def main() -> None:
    ...
    # COST500 is an external final-test set and therefore does not
    # carry the V64 point_role metadata. The inherited evaluator
    # expects its input rows to be tagged as validation internally.
    # This is an in-memory compatibility label only: the sealed
    # COST500 reference file is never modified.
    if "point_role" not in validation.columns:
        validation["point_role"] = "validation"
    ...
    if not validation[
        ...
        raise RuntimeError(
            "Internal COST500 compatibility point_role is invalid."
        )
    ...
    if len(result) != 500:
        raise RuntimeError(
            f"Expected 500 final predictions, "
            f"got {len(result)}."
        )
    ...
    pred_path = (
        out_root
        / f"N{budget}_cost500_predictions_500.csv"
    )

    global_path = (
        out_root
        / f"N{budget}_cost500_metrics_global.json"
    )

    chart_path = (
        out_root
        / f"N{budget}_cost500_metrics_by_chart.csv"
    )

    worst_path = (
        out_root
        / f"N{budget}_cost500_worst_points.csv"
    )
