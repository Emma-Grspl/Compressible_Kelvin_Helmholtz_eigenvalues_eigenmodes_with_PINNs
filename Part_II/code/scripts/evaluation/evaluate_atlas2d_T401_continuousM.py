...
VALIDATION_FILE = (
    REPO / 'article/tables/table_test_reference_401_SEALED.csv'
)

CHECKPOINT_SOURCE_PREFIX = "assets/pinn_supersonic/atlas2d_v1_continuousM"

OUTPUT_ROOT = REPO / "experiments/pinn_direct/T401"
...
def main() -> None:
    ...
    # T401 is an external final-test set and therefore does not
    ...
    # T401 reference file is never modified.
    ...
    if len(validation) != 401:
        raise RuntimeError(
            "Expected 401 T401 points, "
    ...
    if not validation[
        ...
        raise RuntimeError(
            "Internal T401 compatibility point_role is invalid."
        )
    ...
    if len(result) != 401:
        raise RuntimeError(
            f"Expected 401 final predictions, "
            f"got {len(result)}."
        )
    ...
    pred_path = (
        out_root
        / f"N{budget}_test401_predictions_401.csv"
    )

    global_path = (
        out_root
        / f"N{budget}_test401_metrics_global.json"
    )

    chart_path = (
        out_root
        / f"N{budget}_test401_metrics_by_chart.csv"
    )

    worst_path = (
        out_root
        / f"N{budget}_test401_worst_points.csv"
    )
