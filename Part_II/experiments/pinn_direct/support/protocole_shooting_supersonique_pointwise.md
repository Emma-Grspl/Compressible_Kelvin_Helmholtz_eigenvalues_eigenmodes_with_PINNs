...
```bash
sbatch code/slurm/classical_convergence/jz_submit_supersonic_shooting_point_batch.slurm
```
...
`Mach = 1.6` est maintenant lui aussi dans la base pointwise robuste.

## Decision de campagne pour les prochains Mach

Tant que la reference centrale ne contient pas encore de Mach au-dessus du Mach cible :

- il ne faut **pas** utiliser le protocole `branch-guided` ;
- il faut utiliser des campagnes **pointwise non-guided**.

En pratique :

- `M = 1.7` et `M = 1.8` doivent etre ouverts en pointwise non-guided ;
- le `branch-guided` ne redevient pertinent qu'une fois ces nouvelles lignes reintegrees dans la reference centrale.

## Cible pratique a 6 semaines

La cible raisonnable pour le benchmark PINN supersonique n'est pas une surface modale dense.

La cible raisonnable est :

- une base `gold` modale de l'ordre de `25-35` points ;
- un squelette spectral plus large sur `Mach in [1.2, 1.8]` ;
- une carte explicite des zones encore ambiguës.
...
```bash
bash code/slurm/generic_shooting/submit_supersonic_pointwise_core_band.sh
bash code/slurm/generic_shooting/submit_supersonic_pointwise_extended_band.sh
```
...
sbatch code/slurm/generic_shooting/jz_submit_supersonic_shooting_point_batch_M140_branch_guided.slurm
```
