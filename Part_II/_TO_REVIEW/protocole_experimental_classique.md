...
Ce document est maintenant complete par :

- [roadmap_6_semaines_references_et_pinn.md](roadmap_6_semaines_references_et_pinn.md)
- [experiments/pinn_direct/support/protocole_shooting_supersonique_pointwise.md](experiments/pinn_direct/support/protocole_shooting_supersonique_pointwise.md)
...
- verrouillage d'une référence subsonique via [hybrid_subsonic_scan.py](classical_solver/subsonic/hybrid_subsonic_scan.py)
- documentation du workflow de référence dans [classical_solver/subsonic/README.md](classical_solver/subsonic/README.md)
- production d'artefacts de référence :
  - [subsonic_hybrid_growth_map.csv](assets/classic_subsonic/data/subsonic_hybrid_growth_map.csv)
  - [subsonic_hybrid_vs_blumen.png](assets/classic_subsonic/plots/subsonic_hybrid_vs_blumen.png)
  - [subsonic_hybrid_error_summary.json](assets/classic_subsonic/data/subsonic_hybrid_error_summary.json)
...
- solveur principal de tir rapide : [shooting_subsonic.py](classical_solver/subsonic/shooting_subsonic.py)
- solveur de contrôle robuste près de la neutralité : [mstab17_subsonic_solver.py](classical_solver/subsonic/mstab17_subsonic_solver.py)
...
- les isolignes subsoniques de Blumen sont effectivement reconstruites classiquement
- on dispose déjà d'une carte dense exploitable pour le benchmark PINN
- on dispose aussi de reconstructions modales de référence sur des cas représentatifs
...
- essentiellement de l'organisation documentaire et des exports propres pour le benchmark PINN
- garder la cohérence entre les métriques de comparaison du classique et celles utilisées côté PINN
- éviter de rouvrir inutilement le chantier de validation du classique subsonique

### Décision de travail à court terme

Dans le mois qui vient :

- le classique subsonique est considéré comme acquis ;
- la priorité n'est plus de le revalider ;
- la priorité est d'utiliser cette référence pour mesurer ce que le PINN sait ou ne sait pas faire.
...
- centralisation des références Blumen dans [blumen_reference.py](code/scripts/evaluation/blumen_reference.py)
- audit des familles contre Blumen avec [audit_supersonic_families_against_blumen.py](code/scripts/audits/audit_supersonic_families_against_blumen.py)
- sweep GEP guidé par Blumen avec [run_supersonic_gep_blumen_guided_sweep.py](code/scripts/legacy/run_supersonic_gep_blumen_guided_sweep.py)
- ajout d'un diagnostic du spectre brut contre Blumen avec [diagnose_supersonic_raw_spectrum_vs_blumen.py](code/scripts/audits/audit_diagnose_supersonic_raw_spectrum_vs_blumen.py)
...
Dans le mois qui vient, la priorité classique supersonique est :

- enrichir la base pointwise robuste sur `Mach in [1.2, 1.8]` ;
- accepter une référence sparse mais fiable ;
- ne pas perdre de temps à forcer une surface modale continue partout.
