...
def main():
    ...
    for epoch in range(1, args.pq_epochs + 1):
        ...
        # Local u-core correction, no classical/modal field anchor.
        # Added after the base p/q physics losses; does not modify the call signature.
        if float(args.w_u_core_smooth) > 0.0 or float(args.w_u_core_amp) > 0.0:
            p_ucore, q_ucore = field=field(y, alpha, mach)
            core_mask = (torch.abs(y) <= float(args.u_core_ymax)).to(y.real.dtype)

            if float(args.w_u_core_smooth) > 0.0:
                qy_ucore = pqmini.grad_complex(q_ucore, y)
                terms["u_core_smooth"] = float(args.w_u_core_smooth) * torch.mean(
                    core_mask * (qy_ucore.real**2 + qy_ucore.imag**2)
                )

            if float(args.w_u_core_amp) > 0.0:
                terms["u_core_amp"] = float(args.w_u_core_amp) * torch.mean(
                    core_mask * (q_ucore.real**2 + q_ucore.imag**2)
                )
