...
Ce protocole doit maintenant etre lu avec la feuille de route :

- [roadmap_6_semaines_references_et_pinn.md](${PROJECT_ROOT}/roadmap_6_semaines_references_et_pinn.md)

Le point strategique est le suivant :

- dans le delai du projet, le **PINN hybride subsonique** reste le resultat operationnel de reference ;
- les variantes purement physiques locales sont des campagnes methodologiques a promouvoir seulement si elles verrouillent a la fois le mode et `c_i`.
...
Le point central a clarifier est le suivant :

- la **reference classique subsonique est deja acquise** ;
- on a deja reconstruit classiquement les isolignes subsoniques de Blumen ;
- on dispose deja d'une carte dense de reference ;
- on dispose aussi de reconstructions modales representatives.

Donc, dans ce document, le subsonique n'est plus un chantier de validation classique.
Le vrai chantier est maintenant :

- comment utiliser cette reference pour reduire autant que possible la supervision classique du PINN sans perdre l'eigenvaleur ni le mode.

Les sections 1D de bootstrap sont maintenant suffisantes pour ouvrir un premier pilote 2D :

- reference figee `M=0.5` :
  - [frozen_M05_riccati_reference_current](${PROJECT_ROOT}/assets/pinn_subsonic/mach_fixed/frozen_M05_riccati_reference_current)
- section validee `M=0.6` :
  - [experiment_M06_mode_repair_edges_2026-06-05](${PROJECT_ROOT}/assets/pinn_subsonic/experiment_M06_mode_repair_edges_2026-06-05)
...
- [run_kh_subsonic_singlecase_pure_physics_scalar_ci_matching.sh](${PROJECT_ROOT}/code/src/scripts/utils/run_kh_subsonic_singlecase_pure_physics_scalar_ci_matching.sh)
...
Retour d'experience numerique sur le premier run agressif :

- la version initiale de `Phase A-ter` lancait `L_shoot_path` **des l'epoch 1**, **a chaque epoch**, avec une trajectoire allant presque jusqu'au bord compactifie ;
- dans cette configuration, le cout de calcul devient tres eleve, car le terme de trajectoire demande de nombreuses integrations RK4 supplementaires avec graphe autodiff ;
- en pratique, on a observe un run tres lent et instable, avec explosion numerique puis `nan` ;
- ce resultat ne veut pas dire que l'idee de `L_shoot_path` est mauvaise ;
- il montre surtout qu'il faut l'utiliser comme **contrainte de forme modale secondaire**, pas comme verrou spectral brutal des le debut.

Regle retenue pour les prochains essais `Phase A-ter` :

1. garder `L_shoot` et `L_ci_local_min` comme contraintes principales sur `c_i` ;
2. faire agir `L_shoot_path` plus tard dans l'entrainement ;
3. le calculer moins souvent ;
4. le limiter a une zone plus interieure ;
5. reduire le nombre de points de trajectoire.

Configuration de stabilisation retenue par defaut :

- `w_riccati_shooting_path = 5.0`
- `riccati_shooting_path_points = 17`
- `riccati_shooting_path_xi_boundary = 0.94`
- `riccati_shooting_path_start_epoch = 200`
- `riccati_shooting_path_every = 10`

Interpretation :

- `L_shoot_path` doit aider a aligner la **forme** de `gamma(y)` avec une trajectoire de shooting compatible avec le `c_i` courant ;
- il ne doit pas, a lui seul, porter tout le probleme de selection de l'eigenvaleur des le debut de l'optimisation.
...
## Decision de gestion du temps

Sur l'horizon d'environ 6 semaines :

- on ne doit pas laisser les campagnes purement physiques locales retarder la consolidation du baseline hybride subsonique ;
- la bonne discipline est :
  1. conserver une version hybride 2D exploitable ;
  2. tester `Phase A-ter` ;
  3. si `Phase A-ter` echoue encore, passer a une nouvelle idee structurelle, pas a une longue suite de petits tunings.

Autrement dit :

- **un test pur PINN negatif est une information scientifique utile** ;
- il ne doit pas devenir un puits de temps qui empeche de livrer une surface subsonique exploitable.
...
- runner :
  - [run_kh_subsonic_pinn_2d_pilot_M05_M06.py](${PROJECT_ROOT}/archive/code/scripts/run_kh_subsonic_pinn_2d_pilot_M05_M06.py)
- launcher Jean Zay :
  - [jz_submit_kh_subsonic_pinn_2d_pilot_M05_M06.slurm](${PROJECT_ROOT}/code/src/launch/slurm/jz_submit_kh_subsonic_pinn_2d_pilot_M05_M06.slurm)
