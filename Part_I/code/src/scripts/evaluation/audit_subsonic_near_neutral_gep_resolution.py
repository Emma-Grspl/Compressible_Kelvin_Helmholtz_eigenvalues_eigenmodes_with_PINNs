...
def run_one(row, n_points, xi_max):
    mach = float(row["Mach"])
    eta = float(row["eta"])
...
def main():
    ...
    targets = df[
        (df["eta"] >= 0.9575) &
        (df["Mach"].isin([0.10, 0.40, 0.70]))
    ].copy()

    rows = []
    for _, row in targets.iterrows():
        for n_points in [301, 401, 501]:
            for xi_max in [0.98, 0.995]:
                print(f"[RUN] M={row['Mach']} eta={row['eta']} N={n_points} xi={xi_max}")
                out = run_one(row, n_points=n_points, xi_max=xi_max)
                rows.append(out)
                print(
                    f"  success={out['success']} "
                    f"ci_rel={out['ci_gep_rel_err']} "
                    f"p={out['p_rel']} u={out['u_rel']} v={out['v_rel']} "
                    f"source={out['selection_source']}"
                )

    res = pd.DataFrame(rows)
    outdir = ROOT / "assets/pinn_subsonic/local_atlas_v1/gep_near_neutral_resolution_audit"
    outdir.mkdir(parents=True, exist_ok=True)

    path = outdir / "summary_gep_near_neutral_resolution.csv"
    res.to_csv(path, index=False)

    print("\n===== summary =====")
    print(res.sort_values(["Mach","eta","N","xi_max"]).to_string(index=False))
    print("\nCSV:", path)
