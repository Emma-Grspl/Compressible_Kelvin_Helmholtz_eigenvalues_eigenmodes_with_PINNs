...
from src.scripts.classical.solve_shooting_subsonic import sample_subsonic_growth_map
...
def plot_map(df: pd.DataFrame, curves: list[dict], output_path: Path) -> None:
    ...
    digitization_err_mach = 1.0e-2
    digitization_err_alpha = 1.0e-2

    for curve in curves:
        ax.errorbar(
            curve["data"]["Mach"],
            curve["data"]["alpha"],
            xerr=digitization_err_mach,
            yerr=digitization_err_alpha,
            fmt="o",
            markersize=4,
            elinewidth=0.8,
            capsize=1.8,
    ...
    ax.plot(mach_line, alpha_line, "--", color="black", linewidth=1.8, label=r"Neutral boundary $\alpha^2 + M^2 = 1$")

    ax.set_title("Blumen 1970: subsonic reconstruction by shooting")
