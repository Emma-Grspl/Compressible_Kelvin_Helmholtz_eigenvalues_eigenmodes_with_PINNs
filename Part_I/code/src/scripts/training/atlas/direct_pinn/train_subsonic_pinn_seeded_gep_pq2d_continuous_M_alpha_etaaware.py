...
class FieldPQNet(nn.Module):
    """
    Eta-aware p/q network.

    Same external call signature as the previous FieldPQNet:
        field(y, alpha, mach) -> p, q

    But internally it uses the natural subsonic coordinate:
    ...
    def __init__(
        ...
        # z, alpha_norm, mach_norm, eta_norm, alpha_cut_norm,
        # tanh(alpha*y), tanh(alpha_cut*y)
        in_dim = 7 + 2 * n_freq
    ...
    def features(self, y: torch.Tensor, alpha: torch.Tensor, mach: torch.Tensor) -> torch.Tensor:
        ...
        # alpha_cut ranges approximately in [0,1]. Put it on roughly [-1,1].
        k = 2.0 * alpha_cut - 1.0

        # Additional shape coordinates. These help distinguish broad/compact modes.
        ay = torch.tanh(alpha * y)
        ky = torch.tanh(alpha_cut * y)

        feats = [z, a, m, e, k, ay, ky]

        for j in range(1, self.n_freq + 1):
            feats.append(torch.sin(math.pi * j * z))
            feats.append(torch.cos(math.pi * j * z))
