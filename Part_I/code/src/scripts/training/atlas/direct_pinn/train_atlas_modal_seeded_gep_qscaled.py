...
def physics_losses_scaledQ(
    ...
    """
    Low-alpha scaled formulation.

    The network still returns two complex fields (p, Qraw), but Qraw is
    interpreted as Q = q / alpha.

    q = alpha Q
    p_y = alpha Q
    Q_y - (2 U_y / D) Q - alpha(1 - M^2 D^2)p = 0
    """
    ...
    U = torch.tanh(y)
    Uy = 1.0 - U**2
    D = torch.complex(U, torch.zeros_like(U)) - c

    ones = torch.ones_like(y)
    dp_re = torch.autograd.grad(p.real, y, grad_outputs=ones, create_graph=True, retain_graph=True)[0]
    dp_im = torch.autograd.grad(p.imag, y, grad_outputs=ones, create_graph=True, retain_graph=True)[0]
    dQ_re = torch.autograd.grad(Q.real, y, grad_outputs=ones, create_graph=True, retain_graph=True)[0]
    dQ_im = torch.autograd.grad(Q.imag, y, grad_outputs=ones, create_graph=True, retain_graph=True)[0]

    dp = torch.complex(dp_re, dp_im)
    dQ = torch.complex(dQ_re, dQ_im)

    alpha_c = torch.complex(alpha, torch.zeros_like(alpha))

    r_compat = dp / alpha_c - Q
    r_ode = dQ - (2.0 * torch.complex(Uy, torch.zeros_like(Uy)) / D) * Q - alpha_c * (1.0 - mach**2 * D**2) * p

    # Boundary points. Use scaled outgoing BC:
    # Q_L - lambda_L/alpha p_L = 0
    # Q_R + lambda_R/alpha p_R = 0
    yL = -ymax * torch.ones_like(alpha, requires_grad=True)
    yR = ymax * torch.ones_like(alpha, requires_grad=True)

    pL, QL = field(yL, alpha, mach)
    pR, QR = field(yR, alpha, mach)

    D_left = torch.complex(-torch.ones_like(alpha), torch.zeros_like(alpha)) - c
    D_right = torch.complex(torch.ones_like(alpha), torch.zeros_like(alpha)) - c

    lam_left = alpha_c * torch.sqrt(1.0 - mach**2 * D_left**2)
    lam_right = alpha_c * torch.sqrt(1.0 - mach**2 * D_right**2)

    bcL = QL - (lam_left / alpha_c) * pL
    bcR = QR + (lam_right / alpha_c) * pR

    # Gauge at y=0.
    y0 = torch.zeros_like(alpha, requires_grad=True)
    p0, Q0 = field(y0, alpha, mach)

    gauge = (
        torch.mean((p0.real - 1.0) ** 2)
        + torch.mean(p0.imag**2)
    )

    q_center = torch.mean(Q0.real**2)

    return {
        "compat": torch.mean(torch.abs(r_compat) ** 2),
        "ode": torch.mean(torch.abs(r_ode) ** 2),
        "bc": torch.mean(torch.abs(bcL) ** 2) + torch.mean(torch.abs(bcR) ** 2),
        "gauge": gauge,
        "q_center": q_center,
...
def eval_model_scaledQ(field, ci_net, y_np, alpha, Mach, device):
    dtype = torch.float64
    y = torch.tensor(y_np, device=device, dtype=dtype).view(-1, 1)
...
def run_diagnostics_eta(args, field, ci_provider, anchor_df, device):
    ...
    for Mach in eval_mach_values:
        for eta in eval_eta_values:
            ...
            p_pred, q_pred, ci_pred = eval_model_scaledQ(field, ci_provider, y, alpha, Mach, device)
...
def main():
    ...
    with open(outdir / "README.md", "w") as f:
        ...
        f.write("- trained outputs: Re p, Im p, Re Q, Im Q with q = alpha Q\n")
