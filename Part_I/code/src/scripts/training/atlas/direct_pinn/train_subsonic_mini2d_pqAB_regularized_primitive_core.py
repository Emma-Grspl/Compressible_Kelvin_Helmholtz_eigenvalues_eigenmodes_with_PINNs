...
class FieldPQNet(nn.Module):
    ...
    def forward_full(self, y: torch.Tensor, alpha: torch.Tensor, mach: torch.Tensor):
        """Return p, q, A, B where A = D v and B = D^2 u."""
        out = self.net(self.features(y, alpha, mach))
        pr, pi = out[:, 0:1], out[:, 1:2]
        qr, qi = out[:, 2:3], out[:, 3:4]
        ar, ai = out[:, 4:5], out[:, 5:6]
        br, bi = out[:, 6:7], out[:, 7:8]
        p = torch.complex(pr, pi)
        q = torch.complex(qr, qi)
        Areg = torch.complex(ar, ai)
        Breg = torch.complex(br, bi)
        return p, q, Areg, Breg

    def forward(self, y: torch.Tensor, alpha: torch.Tensor, mach: torch.Tensor):
        # Compatibility path for existing warm-start / p-q code.
        p, q, _, _ = self.forward_full(y, alpha, mach)
        return p, q
...
def physics_losses(
    ...
    # Regularized primitive formulation:
    #   A = D v
    #   B = D^2 u
    #   D = U - c
    #
    ...
    #   R_q    = p_y - q
    #   R_v    = i alpha A + q
    #   R_u    = B + D p + U_y A/(i alpha)
    #   R_cont = D A_y - U_y A + i alpha B + i alpha M^2 D^3 p
    p, q, Areg, Breg = field.forward_full(y, alpha, mach)
    ...
    D = Uc - c
    ialpha = torch.complex(torch.zeros_like(alpha), alpha)

    r_compat = dp - q
    r_v = ialpha * Areg + q
    r_u = Breg + D * p + Upc * Areg / ialpha
    r_cont = D * dA - Upc * Areg + ialpha * Breg + ialpha * mach**2 * D**3 * p

    loss_compat = complex_mse(r_compat)
    loss_v = complex_mse(r_v)
    loss_u = complex_mse(r_u)
    loss_cont = complex_mse(r_cont)

    # Keep key "ode" so the existing training loop/wrapper remains compatible.
    loss_ode = loss_cont + 10.0 * loss_v + 10.0 * loss_u

    y0 = torch.zeros_like(y)
    y0.requires_grad_(True)
    p0, q0, _, _ = field.forward_full(y0, alpha, mach)
    ...
    pL, qL, _, _ = field.forward_full(yL, alpha, mach)
    pR, qR, _, _ = field.forward_full(yR, alpha, mach)
...
def eval_model(field: FieldPQNet, ci_net: CiNet, y_np: np.ndarray, alpha: float, Mach: float, device):
    ...
    with torch.no_grad():
        p, q, Areg, Breg = field.forward_full(y, a, m)
        ci_pred = ci_net(a[:1], m[:1])

    return (
        p.cpu().numpy().reshape(-1),
        q.cpu().numpy().reshape(-1),
        Areg.cpu().numpy().reshape(-1),
        Breg.cpu().numpy().reshape(-1),
...
def run_diagnostics(args, field, ci_net, anchor_df, device):
    ...
    for Mach in mach_values:
        for alpha in eval_alphas:
            ...
            p_pred, q_pred, A_pred, B_pred, ci_pred = eval_model(field, ci_net, y, alpha, Mach, device)
            ...
            rho_pred = Mach**2 * p_pred
            v_pred = A_pred / D_pred
            u_pred = B_pred / D_pred**2
            gamma_pred = q_pred / np.where(np.abs(p_pred) > 1e-14, p_pred, np.nan + 1j * np.nan)
            ...
            row = {
                ...
                "A_Dv_rel": rel_l2(A_pred, A_ref, y, mask),
                "B_D2u_rel": rel_l2(B_pred, B_ref, y, mask),
            ...
            plot_complex_pair(y, A_ref, A_pred, f"Regularized A=Dv, M={Mach:g}, alpha={alpha:g}", outdir / f"regularized_A_Dv_{tag}.png", "A")
            plot_complex_pair(y, B_ref, B_pred, f"Regularized B=D2u, M={Mach:g}, alpha={alpha:g}", outdir / f"regularized_B_D2u_{tag}.png", "B")
            ...
            pd.DataFrame({
                ...
                "A_Dv_ref_real": np.real(A_ref),
                "A_Dv_ref_imag": np.imag(A_ref),
                "A_Dv_pred_real": np.real(A_pred),
                "A_Dv_pred_imag": np.imag(A_pred),

                "B_D2u_ref_real": np.real(B_ref),
                "B_D2u_ref_imag": np.imag(B_ref),
                "B_D2u_pred_real": np.real(B_pred),
                "B_D2u_pred_imag": np.imag(B_pred),
