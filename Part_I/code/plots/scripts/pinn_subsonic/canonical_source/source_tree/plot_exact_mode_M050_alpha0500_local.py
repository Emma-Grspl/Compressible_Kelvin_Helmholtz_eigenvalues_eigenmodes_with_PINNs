...
def load_joint_checkpoint(
    ...
    expected_etaaware = 7 + 2 * n_freq
    if input_dimension != expected_etaaware:
        raise RuntimeError(
            "The chart routed at alpha=0.5, M=0.5 is not the eta-aware "
            "field architecture expected by this standalone script.\n"
            f"checkpoint={checkpoint_path}\n"
            f"input_dimension={input_dimension}, "
            f"eta-aware expected={expected_etaaware}\n"
            "Use the checkpoint metadata to identify the chart family."
        )
...
def plot_mode(
    ...
    for axis, (field, title) in zip(axes, panels):
        ...
        axis.axhline(0.0, color="0.72", linewidth=0.7)
    ...
    fig.tight_layout(rect=(0.025, 0.025, 0.975, 0.87))
