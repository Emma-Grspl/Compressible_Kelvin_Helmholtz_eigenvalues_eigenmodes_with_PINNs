...
MAP1000 = (
    RUNTIME_DIR
    / "map_1000_ci_checks.csv"
)

SELECTION_SUMMARY = (
    ROOT
    / "assets/pinn_subsonic/article_work/"
    "gep_selection_N340_frozen/"
    "summary.csv"
)
...
def plot_heatmap():
    ...
    err = np.abs(
        df["hybrid_ci"].to_numpy(float)
        - df["classical_ci"].to_numpy(float)
    )

    logerr = np.log10(
        np.maximum(err, 1.0e-8)
    )
    ...
    levels = np.linspace(
        max(-8.0, np.nanmin(logerr)),
        min(0.0, np.nanmax(logerr)),
        36,
    )
    ...
    Mline = np.linspace(0, 1, 500)

    ax.plot(
        Mline,
        np.sqrt(
            np.clip(1-Mline**2, 0, None)
        ),
        "--",
        lw=1.3,
    )
...
def plot_neutral_scaling():
    ...
    ax.plot(
        ...
        out["gep_p90"],
        "--s",
        label="Selected GEP: 90th percentile",
    )
...
def plot_budget_comparison():
    ...
    ax.set_title(
        "Selected GEP accuracy and branch robustness"
    )
