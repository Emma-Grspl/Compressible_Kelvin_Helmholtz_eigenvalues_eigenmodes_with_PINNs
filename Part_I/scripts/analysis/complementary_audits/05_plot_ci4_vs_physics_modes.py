...
def save_ci_curve_and_heatmaps(
    ...
    ax_curve.plot(alpha_values, ci_ref, color="black", linewidth=2.2, label="Classical branch")
    ax_curve.plot(alpha_values, ci_hybrid, "--", linewidth=2.0, color="#0b6e4f", label="PINN hybride, 4 points")
    ax_curve.plot(alpha_values, ci_physics, "-.", linewidth=2.0, color="#c84c09", label="Physics only PINN")
    ...
    ax_err_h.set_yticks([])
    ax_err_h.set_title(r"Absolute error heatmap, Four anchor PINN")
    ...
    ax_err_p.set_title(r"Absolute error heatmap, Physics only PINN")
    cbar_p = fig.colorbar(im_p, ax=ax_err_p, pad=0.02)
    cbar_p.set_label(r"$|c_i - c_i^{ref}|$")

    fig_path = output_dir / "ci_curve_hybrid4_vs_physics_only.png"
    fig.savefig(fig_path, bbox_inches="tight")
    plt.close(fig)

    csv_path = output_dir / "ci_curve_hybrid4_vs_physics_only.csv"
    pd.DataFrame(
        {
            "alpha": alpha_values,
            "ci_reference": ci_ref,
            "ci_hybrid_4pt": ci_hybrid,
            "ci_physics_only": ci_physics,
            "err_hybrid_4pt": err_h,
...
def save_mode_comparison(
    *,
    ...
    axes[0].plot(y_common, amp_classic, color="black", linewidth=2.2, label="Classical branch")
    axes[0].plot(y_common, amp_h, "--", color="#0b6e4f", linewidth=2.0, label="PINN hybride, 4 points")
    axes[0].plot(y_common, amp_p, "-.", color="#c84c09", linewidth=2.0, label="Physics only PINN")
    axes[0].set_title(r"Amplitude $|\hat{p}|$")
    axes[0].set_ylabel("Normalised amplitude")
    axes[0].set_xlim(x_min, x_max)
    axes[0].legend()

    axes[1].plot(y_common, phase_classic, color="black", linewidth=2.2, label="Classical branch")
    axes[1].plot(y_common, phase_h, "--", color="#0b6e4f", linewidth=2.0, label="PINN hybride, 4 points")
    axes[1].plot(y_common, phase_p, "-.", color="#c84c09", linewidth=2.0, label="Physics only PINN")
    ...
    metrics = {
        ...
        "ci_hybrid_4pt": float(ci_h),
        "ci_physics_only": float(ci_p),
        "amp_rel_hybrid_4pt": amp_rel_h,
        "amp_rel_physics_only": amp_rel_p,
        "phase_rmse_hybrid_4pt": phase_rmse_h,
...
def save_mode_error_vs_alpha(
    *,
    ...
    axes[0].plot(df["alpha"], df["amp_rel_hybrid_4pt"], "--", color="#0b6e4f", linewidth=2.0, label="PINN hybride, 4 points")
    axes[0].plot(df["alpha"], df["amp_rel_physics_only"], "-.", color="#c84c09", linewidth=2.0, label="Physics only PINN")
    axes[0].set_ylabel("Relative amplitude error")
    axes[0].set_title(r"Mode reconstruction error vs $\alpha$ at fixed Mach")
    axes[0].legend()

    axes[1].plot(df["alpha"], df["phase_rmse_hybrid_4pt"], "--", color="#0b6e4f", linewidth=2.0, label="PINN hybride, 4 points")
    axes[1].plot(df["alpha"], df["phase_rmse_physics_only"], "-.", color="#c84c09", linewidth=2.0, label="Physics only PINN")
...
def save_summary_barplot(summary_df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    width = 0.36
    ax.bar(x - width / 2, summary_df["err_hybrid_4pt"], width=width, color="#0b6e4f", label="PINN hybride, 4 points")
    ax.bar(x + width / 2, summary_df["err_physics_only"], width=width, color="#c84c09", label="Physics only PINN")
...
def main() -> None:
    ...
    hybrid_run = args.base_dir / "hybrid_4pt"
    ...
    budget_summary = pd.DataFrame(
        {
            ...
            "err_hybrid_4pt": [np.nan, float(np.mean(np.abs(ci_hybrid - ci_ref)))],
    ...
    comparison_summary_path = args.base_dir / "comparison_summary.csv"

    if not comparison_summary_path.exists():
        print(
            f"Optional comparison summary absent: "
            f"{comparison_summary_path}. "
            "The prediction and modal figures have already been generated; "
            "skipping only the final summary export."
        )
        return
    ...
    bar_df = pd.DataFrame(
        {
            ...
            "err_hybrid_4pt": [
                np.nan if label != "hybrid_4pt" else val
